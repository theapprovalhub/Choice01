...
const Applications = {
  ...
  async getStatus(appId)        {
    const { data, error } = await sb().rpc('get_application_status', { p_app_id: appId });
    if (error) return { success: false, error: error.message };
    // lease_pdf_url stores the storage path — generate a fresh signed URL on every load.
  ...
  async signLease(appId, sig, ip)           { return callEdgeFunction('sign-lease', { app_id: appId, signature: sig, ip_address: ip }); },
...
const Inquiries = {
  ...
  async getForLandlord(landlordId) {
    const { data, error } = await sb().from('inquiries').select('*, properties(title, address)').in('property_id',
      (await sb().from('properties').select('id').eq('landlord_id', landlordId)).data?.map(p => p.id) || []
